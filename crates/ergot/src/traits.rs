...
pub trait Endpoint {
    ...
    const RESP_KEY: Key;
    /// Traffic class hint for requests to this endpoint; responses inherit it.
    const CLASS: crate::TrafficClass = crate::TrafficClass::Normal;
}
...
pub trait Topic {
    ...
    const TOPIC_KEY: Key;
    /// Traffic class hint for messages on this topic.
    const CLASS: crate::TrafficClass = crate::TrafficClass::Normal;
}
...
/// ```
///
/// If the path is omitted, the type name is used instead. A traffic class
/// hint can be given after the path: `endpoint!(E, Req, Resp, "path", class =
/// TrafficClass::Control)`; the default is `Normal`.
#[macro_export]
macro_rules! endpoint {
    ($tyname:ident, $req:tt $(< $req_lt:lifetime >)?, $resp:tt $(< $resp_lt:lifetime >)?, $path:expr, class = $class:expr $(,)?) => {
        ...
        impl < $($req_lt,)? $($resp_lt)? > $crate::traits::Endpoint for $tyname < $($req_lt,)? $($resp_lt)? > {
            ...
            const CLASS: $crate::TrafficClass = $class;
        }
    };
...
///
/// If the path is omitted, the type name is used instead. A traffic class
/// hint can be given after the path: `topic!(T, Msg, "path", class =
/// TrafficClass::Bulk)`; the default is `Normal`.
#[macro_export]
macro_rules! topic {
    ($tyname:ident, $msg:tt $(< $msg_lt:lifetime >)?, $path:expr, class = $class:expr $(,)?) => {
...
mod test {
    ...
    #[test]
    fn traffic_class_defaults_to_normal_and_is_settable() {
        use crate::TrafficClass;
        topic!(PlainTopic, u32, "class/plain");
        topic!(BulkTopic, u32, "class/bulk", class = TrafficClass::Bulk);
        topic!(
            BulkTopicTrailing,
            u32,
            "class/bulk2",
        ...
        topic!(SamePathNormal, u32, "class/bulk");
        endpoint!(PlainEndpoint, u32, u32, "class/plain/ep");
        endpoint!(
            ControlEndpoint,
            u32,
            u32,
        ...
        assert_eq!(PlainTopic::CLASS, TrafficClass::Normal);
        assert_eq!(BulkTopic::CLASS, TrafficClass::Bulk);
        assert_eq!(BulkTopicTrailing::CLASS, TrafficClass::Bulk);
        assert_eq!(PlainEndpoint::CLASS, TrafficClass::Normal);
        assert_eq!(ControlEndpoint::CLASS, TrafficClass::Control);
        // The class is a hint carried beside the key, not part of it: the same
        // path/type pair keys identically regardless of class.
        assert_eq!(BulkTopic::TOPIC_KEY, SamePathNormal::TOPIC_KEY);
    }
